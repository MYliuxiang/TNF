...
@implementation BgView3

- (instancetype)initWithFrame:(CGRect)frame Title:(NSString *)title ImgName:(NSString *)imgName bgviewdelegate:(id<BgView3delegate>)delegate Text:(NSString *)text Text1:(NSString*)text1{
    ...
    if (self) {
        ...
        UIView *whiltBgView = [[UIView alloc]initWithFrame:CGRectMake((kScreenWidth-(ratioWidth*540/2))/2 ,(kScreenHeight-(ratioHeight*600/2))/2 - 32, ratioWidth*540/2, ratioHeight*600/2)];
...
//        imgView.backgroundColor = [UIColor darkGrayColor];
        ...
        UILabel *TestTime = [WXLabel UIlabelFrame:CGRectMake(0, 30*ratioWidth/2,view.width, 70*ratioHeight/2) textColor:[UIColor whiteColor] textFont:[UIFont systemFontOfSize:40*ratioHeight/2]labelTag:6];
        TestTime.text = [NSString stringWithFormat:@"-%@",text];
        ...
        CGFloat height = [WXLabel getTextHeight:13*ratioHeight width:whiltBgView.width-200*ratioWidth/2 text:text1 linespace:10];
        ...
        [enterButton setTitle:@"立即解锁" forState:UIControlStateNormal];
        ...
        UIButton *close = [[UIButton alloc]initWithFrame:CGRectMake(whiltBgView.width-30, 0, 20, 30)];
        [close setImage:[UIImage imageNamed:@"close_01"] forState:UIControlStateNormal];
        [close addTarget:self action:@selector(CloseAction) forControlEvents:UIControlEventTouchUpInside];
        [whiltBgView addSubview:close];
...
- (void)enterAction
{
    [self.BgViewdelegate lingquClik];
     self.hidden = YES;
